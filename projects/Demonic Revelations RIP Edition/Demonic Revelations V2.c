		...
		if (!HUD_IS_FADED())
		{
			if (!IS_GAME_PAUSED())
			{
				if (TestOne == 2)
				{
					...
					{
						print2("<0xFF0000>xTheDevilRazedMe-Tyler Is a Leaking Script Kiddie Bitch</0x>", 6000);
						WAIT(5000);//gonna display this first then do actual print. It's about time for rockstar to get involved. Doubt they will but meh.
						AUDIO_MUSIC_ONE_SHOT("mex_mission_complete_song_01", 0, 0, 0, 0, 0);
						print_10s("Welcome <green>%s</green> to <red>DEMONiC Revelations V2.7</red> \n<blue>Creators: CabooseSayzWTF, Im Foxxyy, & Sockstress</blue> \n<purple>Hold</purple> <rb><purple>and press</purple> <rs><purple>to open", NGTAG);
						TestOne = 3;
					}
				}
			}
