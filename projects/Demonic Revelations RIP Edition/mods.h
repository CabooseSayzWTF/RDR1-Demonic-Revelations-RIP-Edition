...
void AIOBools()
{
	...
	{
		UNK_0x7D6A8D4A(9, 0);
		UNK_0x90CD8795(9, " xANTIxFR33ZEx ");
	}
	if (TODLoop)
	{
...
void Looped_Functions()
{
	...
	if (NET_IS_IN_SESSION())//not the best script cheat menu enabler but it does the job well enough but can bug out prompts and arrows(not that big of a deal tbh everything seems to work fine even when bugged)
	{
		if (UI_ISFOCUSED("OptionsLayer"))
		{
			UNK_0xB3FC8CB7("OptionsTab_Config", "PM_Extras");
			UI_DEACTIVATE("ConfigOption_Controller");
			UI_HIDE("ConfigOption_Controller");
			UI_UNFOCUS("ConfigOption_Controller");
		}
		if (UI_ISFOCUSED("OptionsList_Config"))
		{
			UI_SET_TEXT("Options_Header", "PM_Extras");
			UI_ACTIVATE("ExtrasLayer");
			UI_FOCUS("ExtrasLayer");
			UI_SHOW("ExtrasLayer");
		}
	}
