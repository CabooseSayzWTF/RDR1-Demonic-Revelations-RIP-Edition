	...
	if (EnableThat)
	{
		if (test123 == 0)
		{
			...
			if (MenuOpen)
			{
				...
				{
					//unsigned char* ORIGNAME1 = (unsigned char *)((int)GET_SLOT_NAME(_GET_HOST_SLOT_ID()) - 0x74 + 0x64);//xbox leftover
					stradd_s(Temp, " <blue>Players:</blue>");
					stradd_s(Temp, INT_TO_STRING(GET_NUM_PLAYERS()));
					stradd_s(Temp, " <red>Host:</red>");
					stradd_s(Temp, GET_SLOT_NAME(_GET_HOST_SLOT_ID()));//ps3 shit because im not gonna go look for true name bs
				}
