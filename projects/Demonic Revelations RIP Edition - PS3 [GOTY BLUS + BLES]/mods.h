...
void AIOBools()
{
	...
	if (animalplay)//xbox leftover till actual offset is found
	{
...
{
	//this could be wrong for ps3 idk, this function is dangerous
	//you will crash eventually from using it
	//i never bothered to do this properly
	//the rapid types may need to be changes to long and value adjusted to be max float
	//so for ps3 i left this as is.
	//dont feel bad this same function is dangerous on xbox and crashes as well
	int a = *(int*)Pointer;
	int *b = (int*)a;
	for (int i = 0; i <= 31; i++)
	{
...
void WepEn()
{
	...
	if (cusweps)
	{
		SlotRap(0x0157A82C, 6, 15, 239);
		CannonGun = 1; print2("<green>FingerGuns Enabled - [Gatling, Browning, Cannon + Rapid Fire For Gat/Brown & Buffalo]", 8000);
		//GiveActorAllWeapons(); feel free to add back, some ps3 players would crash because of this because ps3 is too slow at shit.
	}
	if (!cusweps)
	{
		SlotRap(0x0157A82C, 1, 63, 61);
		CannonGun = 0; print2("<red>FingerGuns Disabled", 3000);
	}
