...
void Protections()
{
	if (NET_IS_IN_SESSION())
	{
		...
		{
			//projectile
			*(byte*)0x015BD8C4 = RAND_INT_RANGE(1, 255); *(byte*)0x015BD8E4 = RAND_INT_RANGE(1, 255); *(byte*)0x015BD904 = RAND_INT_RANGE(1, 255); *(byte*)0x015BD924 = RAND_INT_RANGE(1, 255); *(byte*)0x015BD944 = RAND_INT_RANGE(1, 255);
			//melee
			*(byte*)0x015C096C = RAND_INT_RANGE(1, 255); *(byte*)0x015C098C = RAND_INT_RANGE(1, 255);
		}
		if (DamagePro == 2)//damage protect all - OFF
		{
			//projectile
			*(byte*)0x015BD8C4 = 0; *(byte*)0x015BD8E4 = 0; *(byte*)0x015BD904 = 0; *(byte*)0x015BD924 = 0; *(byte*)0x015BD944 = 0;
			//melee
			*(byte*)0x015C096C = 0; *(byte*)0x015C098C = 0;
		}
		if (DamagePro == 3)//ER Protect (Admin only) - Pro2 bool
		{
			*(byte*)0x015BD9A4 = RAND_INT_RANGE(1, 255);//er protection
		}
		if (DamagePro == 4)//ER Protect OFF
		{
			*(byte*)0x015BD9A4 = 0;//er protection
		}
		if (DamagePro == 5)//Explosion FX Protection (all) - Pro3 Bool
		{
			*(byte*)0x015BD964 = RAND_INT_RANGE(1, 255);//RemoteExplosionEvent(FX Protect)
		}
		if (DamagePro == 6)//Explosion FX Protection OFF
		{
			*(byte*)0x015BD964 = 0;//RemoteExplosionEvent(FX Protect)
		}
		if (DamagePro == 7)//AntifreezeV2 - Pro4 bool
		{
			*(byte*)0x015BDC3C = RAND_INT_RANGE(1, 255);//gohactor block//17
		}
		if (DamagePro == 8)//AntifreezeV2 OFF
		{
			*(byte*)0x015BDC3C = 0;//gohactor block
		}
		if (DamagePro == 9)//block votes on
		{
			*(int*)0x00307DE0 = 0x4E800020;//gohactor block
		}
		if (DamagePro == 10)//block votes off
		{
			*(int*)0x00307DE0 = 0xF821FF21;//gohactor block
		}
		if (DamagePro == 11)//self delete protection, causes local session overload if on for too long, locally you will have to server fix etc and or use delete gun to get overload to go away
		{
			*(byte*)0x015BDEA8 = RAND_INT_RANGE(1, 255);
		}
		if (DamagePro == 12)
		{
			*(byte*)0x015BDEA8 = 0;
...
void GiveActorAllWeapons()
{
	if (NET_IS_IN_SESSION())
	{
		*(int*)0x00091C4C = 0x4E800020;//ps3 need to check, offset is from JamesTwt, uncomment if you freeze when giving all weps, also uncomment if you don't auto get ER
	}
	for (int i = 0; i <= 37; i++)
	{
		if (i != 30)
...
void ChangeModel(Actor actor, int actorID)
{
	...
	_CHANGE_ACTOR_MODEL_IN_LAYOUT(FIND_NAMED_LAYOUT("PlayerLayout"), actor, "", actorID, ve20, ve20, 1);
...
void IPG(int SlotID)
{
	...
	char temp6[255];
	//name
	stradd_s(temp, "<0xFF5733>Name:</0x><0x00FFFF>");
	stradd_s(temp, GET_SLOT_NAME(SlotID));//ps3 not gonna go look for true name bs
	//slot
	stradd_s(temp2, "</0x> <0xFF5733>Slot:</0x><0x00FFFF>");
	straddi_s(temp2, SlotID);
	//NPID
	unsigned char* NPIDPrint1 = (unsigned char *)((int)GET_SLOT_NAME(SlotID) - 0x60);
	unsigned char* NPIDPrint2 = (unsigned char *)((int)GET_SLOT_NAME(SlotID) - 0x64);
	stradd_s(temp3, "</0x> <0xFF5733>NPID:</0x><0x00FFFF>");
	stradd_s(temp3, NPIDPrint1);
	stradd_s(temp3, NPIDPrint2);
	stradd_s(temp3, "</0x> ");
	//padding
	stradd_s(temp4, " ");
	stradd_s(temp5, " ");
	...
	{
		_PRINT_CHAT(temp2, 0, 0, 0, 0, 0, 0);//slot
		_PRINT_CHAT(temp, 0, 0, 0, 0, 0, 0);//name
		_PRINT_CHAT(temp3, 0, 0, 0, 0, 0, 0);//NPID
		_PRINT_CHAT(temp4, 0, 0, 0, 0, 0, 0);//padding
		_PRINT_CHAT(temp5, 0, 0, 0, 0, 0, 0);//padding
		_PRINT_CHAT(temp6, 0, 0, 0, 0, 0, 0);//padding
...
void AIOBools()
{
			...
			if (!IS_ACTOR_DEAD(self))
			{
				if (IS_ACTOR_VALID(self))
				{
					if (GET_ACTOR_ENUM(self) != DefautSavedModel)
					{
						...
						//GiveActorAllWeapons(); feel free to add back, some ps3 players would crash because of this because ps3 is too slow at shit.
					}
				}
			}
		}
	...
	if (PLATFORM == PLATFORM_PS3 && (SuperJump) && GetButtonDown(BUTTON_SQUARE) && IS_ACTOR_ALIVE(self))
	{
	...
	if (AntiRagDoll)
	{
		if (!IS_ACTOR_ANIMAL(self))//test to see if we fix menu lockup
		{
	...
	{
		if (!lodmod)//we are gonna ignore demi changemodel fix here if use saved model is on, use saved model will replicate this if demi is on for us
		{
			ChangeModel(self, GET_ACTOR_ENUM(self));//GiveActorAllWeapons(); feel free to add back, some ps3 players would crash because of this because ps3 is too slow at shit.
		}
	}
	if (AnimalBypassON)
	{
...
void Underglow(int x1, int y1, int z1, float x2, float y2, float z2, const char* FXType1, const char* FXType2)
{
	DESTROY_OBJECT(vehglow1); DESTROY_OBJECTSET(vehglow2); DESTROY_OBJECT(glowfx2); DESTROY_OBJECTSET(glowfx1);
	DESTROY_OBJECT(vehglow11); DESTROY_OBJECTSET(vehglow22); DESTROY_OBJECT(glowfx22); DESTROY_OBJECTSET(glowfx11);
	DESTROY_OBJECT(vehglow111); DESTROY_OBJECTSET(vehglow222); DESTROY_OBJECT(glowfx222); DESTROY_OBJECTSET(glowfx111);
	glowfx1 = UNK_0x921B5F2B("emitter_set", GET_VEHICLE(self), -1, 1);
	glowfx2 = UNK_0xFF8CBD07(0, FXType1, GET_VEHICLE(self), TO_FLOAT(x1), TO_FLOAT(y1), TO_FLOAT(z1), 1, GET_VEHICLE(self));
	UNK_0x6745191B(glowfx2, TO_FLOAT(0), TO_FLOAT(0), TO_FLOAT(0));
	UNK_0x43FBBDE1(glowfx2, glowfx1);
	vehglow2 = UNK_0x921B5F2B("emitter_set", GET_VEHICLE(self), -1, 1);
	vehglow1 = UNK_0x2A902148(0, FXType2, "chassisMain_joint", GET_VEHICLE(self), x2, y2, z2, GET_VEHICLE(self));
	UNK_0x6745191B(vehglow1, TO_FLOAT(0), TO_FLOAT(0), TO_FLOAT(0));
	UNK_0x43FBBDE1(vehglow1, vehglow2);
	glowfx11 = UNK_0x921B5F2B("emitter_set", GET_VEHICLE(self), -1, 1);
	glowfx22 = UNK_0xFF8CBD07(0, FXType1, GET_VEHICLE(self), TO_FLOAT(x1), TO_FLOAT(y1), TO_FLOAT(1.0), 1, GET_VEHICLE(self));
	UNK_0x6745191B(glowfx22, TO_FLOAT(0), TO_FLOAT(0), TO_FLOAT(0));
	UNK_0x43FBBDE1(glowfx22, glowfx11);
	vehglow22 = UNK_0x921B5F2B("emitter_set", GET_VEHICLE(self), -1, 1);
	vehglow11 = UNK_0x2A902148(0, FXType2, "chassisMain_joint", GET_VEHICLE(self), x2, y2, z2, GET_VEHICLE(self));
	UNK_0x6745191B(vehglow11, TO_FLOAT(0), TO_FLOAT(0), TO_FLOAT(0));
	UNK_0x43FBBDE1(vehglow11, vehglow22);
	glowfx111 = UNK_0x921B5F2B("emitter_set", GET_VEHICLE(self), -1, 1);
	glowfx222 = UNK_0xFF8CBD07(0, FXType1, GET_VEHICLE(self), TO_FLOAT(x1), TO_FLOAT(y1), TO_FLOAT(-1.0), 1, GET_VEHICLE(self));
	UNK_0x6745191B(glowfx222, TO_FLOAT(0), TO_FLOAT(0), TO_FLOAT(0));
	UNK_0x43FBBDE1(glowfx222, glowfx111);
	vehglow222 = UNK_0x921B5F2B("emitter_set", GET_VEHICLE(self), -1, 1);
	vehglow111 = UNK_0x2A902148(0, FXType2, "chassisMain_joint", GET_VEHICLE(self), x2, y2, z2, GET_VEHICLE(self));
	UNK_0x6745191B(vehglow111, TO_FLOAT(0), TO_FLOAT(0), TO_FLOAT(0));
	UNK_0x43FBBDE1(vehglow111, vehglow222);
}
...
void MainLRSwitches()
{
	if (ToggleSpoof13)
	{
		...
		if (PedWepz == 27)
		{
			stradd(modmsg, "Vehicle Underglow\n<x>:Confirm\n<cancel>:Cancel\n<dpadleftright>:Underglow:<orange> ", 255 - sizeof(modmsg) - 1);
			if (undercolor == 0)stradd(modmsg, "Remove Underglow", 255 - sizeof(modmsg) - 1);
			if (undercolor == 1) { stradd(modmsg, "Green (Dim)", 255 - sizeof(modmsg) - 1); glow = "weap_torch_flame_blueFire"; }
			if (undercolor == 2) { stradd(modmsg, "Red (Bright)", 255 - sizeof(modmsg) - 1); glow = "weap_torch_flame"; }
			if (undercolor == 3) { stradd(modmsg, "Red (Soft)", 255 - sizeof(modmsg) - 1); glow = "muzzle_fire_bottle"; }
		}
		...
		if (PLATFORM == PLATFORM_PS3 && GetButton(BUTTON_SQUARE))
		{
			...
			if (PedWepz == 9)
			{
				...
				//setGlobalAtIndex(16524, weathint);//force host/non host set weather removed, uncomment to add back
			...
			if (PedWepz == 27)
			{
				if (undercolor == 0)
				{
					...
					Underglow(0, 1.0, 0, 3.0f, 5.0, 3.0, glow, glow);
			}
		}
		if (GetButton(BUTTON_B))
		{
...
{
	cusweps = !cusweps;
	//this is horribly wrong and could cause issues, however if max rank online, this SHOULD make most weapons allow for shooting fast af.
	//throwables dont get rapid fire, one of the snipers dont get rapid fire, and I think volcanic doesnt get rapid fire
	//the above function SlotRap controls the weapon memory shit, improve it if you wish
	if (cusweps) { SlotRap(0x0157A82C, 6, 20000000, 0); }
	if (!cusweps) { SlotRap(0x0157A82C, 1, 63, 61); }
	if (cusweps) { CannonGun = 1; print2("<green>ModWeps Enabled - [Gatling, Browning, Cannon + Rapid Tap Fire For Most Weapons</green> \n <purple>Best With a Modded Controller + Inf Ammo/No Reload On", 11000); }
...
void ModdedTitle()
{
	if (modtit == 0) { setGlobalAtIndex(76886, 4009); print2("Title Set To Tumbleweed(unused title)", 5000); modtit = 1; }
	else if (modtit == 1) { setGlobalAtIndex(76886, 4010); print2("Title Set To Speedy(unused title)", 5000); modtit = 2; }
	else if (modtit == 2) { setGlobalAtIndex(76886, 2025); print2("Title Set To The Pirate(unused title)", 5000); modtit = 3; }
	else if (modtit == 3) { setGlobalAtIndex(76886, 51); print2("Title Set To The Prestige(unused title)", 5000); modtit = 4; }
	else if (modtit == 4) { setGlobalAtIndex(76886, 8021); print2("Title Set To Shifty(unused title)", 5000); modtit = 5; }
	else if (modtit == 5) { setGlobalAtIndex(76886, 8022); print2("Title Set To Untrustworthy(unused title)", 5000); modtit = 6; }
	else if (modtit == 6) { setGlobalAtIndex(76886, 8023); print2("Title Set To Born Liar(unused title)", 5000); modtit = 0; }
}
...
void Looped_Functions()
{
	if (MSpeed)
	{
		if (IS_ACTOR_RIDING(self) || IS_ACTOR_DRIVING_VEHICLE(self))
		{
			if (MSpeed2 == 0)
			{
				*(long*)0x00BE47DC = 1090519040;
				MSpeed2 = 1;
			}
		}
		if (!IS_ACTOR_RIDING(self) && !IS_ACTOR_DRIVING_VEHICLE(self))
