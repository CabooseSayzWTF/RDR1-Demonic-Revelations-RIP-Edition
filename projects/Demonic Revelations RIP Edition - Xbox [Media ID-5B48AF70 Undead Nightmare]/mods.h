...
//RETAIL FLAG - Change to 1 if this will be a retail console script
bool RETAILXBOX = 0;
//
...
void Protections()
{
	if (RETAILXBOX)//we are gonna force some protections off to protect retail side from instant crashing
	{
		Pro5 = false;
		Pro6 = false;
	}
...
void AIOBools()
{
	...
	if (SpoofDe)//useless now, was useless when source was released
	{
		for (int i = 0; i <= 15; i++)
		{
			if (IS_SLOT_VALID(i))
	...
	if (AntiRagDoll)
	{
		if (!IS_ACTOR_ANIMAL(self))//test to see if we fix menu lockup, we ignore no ragdoll if it's on
		{
	...
	if (TODLoop2)
	{
		//region host/2nd host of weather removed so people dont try to
		//become 2nd host of weather anymore, mostly so people can locally change weather only for them
		//uncomment to add back
...
void ModdedTitle()
{
	...
	else if (modtit == 5) { setGlobalAtIndex(76886, 4010); print2("Title Set To Speedy(unused title)", 5000); modtit = 6; }
	else if (modtit == 6) { setGlobalAtIndex(76886, 2025); print2("Title Set To The Pirate(unused title)", 5000); modtit = 7; }
	else if (modtit == 7) { setGlobalAtIndex(76886, 51); print2("Title Set To The Prestige(unused title)", 5000); modtit = 8; }
	else if (modtit == 8) { setGlobalAtIndex(76886, 8021); print2("Title Set To Shifty(unused title)", 5000); modtit = 9; }
	else if (modtit == 9) { setGlobalAtIndex(76886, 8022); print2("Title Set To Untrustworthy(unused title)", 5000); modtit = 10; }
	else if (modtit == 10) { setGlobalAtIndex(76886, 8023); print2("Title Set To Born Liar(unused title)", 5000); modtit = 0; }
}
